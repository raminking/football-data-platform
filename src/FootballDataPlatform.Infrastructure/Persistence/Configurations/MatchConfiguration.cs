...
public sealed class MatchConfiguration : IEntityTypeConfiguration<Domain.Match.Match>
{
    public void Configure(EntityTypeBuilder<Domain.Match.Match> builder)
    {
        ...
        builder.HasOne<Season>().WithMany().HasForeignKey(x => x.SeasonId).OnDelete(DeleteBehavior.Restrict).IsRequired();
        builder.HasOne<Team>().WithMany().HasForeignKey(x => x.HomeTeamId).OnDelete(DeleteBehavior.Restrict).IsRequired();
        builder.HasOne<Team>().WithMany().HasForeignKey(x => x.AwayTeamId).OnDelete(DeleteBehavior.Restrict).IsRequired();
