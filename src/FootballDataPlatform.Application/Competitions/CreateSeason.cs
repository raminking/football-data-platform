...
public record CreateSeasonCommand(long CompetitionId, string Name, DateOnly StartDate, DateOnly EndDate) : IRequest<Result<Guid>>;
internal sealed class CreateSeasonHandler(ISeasonRepository repository) : IRequestHandler<CreateSeasonCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(CreateSeasonCommand command, CancellationToken ct)
    {
        if (command.CompetitionId <= 0) return Result<Guid>.Failure("Competition is required.");
        ...
        if (!await repository.CompetitionExistsAsync(command.CompetitionId, ct)) return Result<Guid>.Failure("Competition not found.");
        var name = command.Name.Trim();
        if (await repository.ExistsByIdentityAsync(command.CompetitionId, name, null, ct)) return Result<Guid>.Failure("Season already exists for this competition.");
