...
public record UpdateCompetitionCommand(Guid Id, string Name, string Country, string Code) : IRequest<Result<Guid>>;

internal sealed class UpdateCompetitionHandler(ICompetitionRepository repository)
    : IRequestHandler<UpdateCompetitionCommand, Result<Guid>>
{
    public async Task<Result<Guid>> Handle(UpdateCompetitionCommand command, CancellationToken ct)
    {
        ...
        if (await repository.ExistsByIdentityAsync(name, country, code, command.Id, ct))
            return Result<Guid>.Failure("Competition already exists.");
