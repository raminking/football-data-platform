...
public class CompetitionsApiTests
{
    ...
    {
        await using var factory = new CustomWebApplicationFactory(); using var client = factory.CreateClient(); var request = new CreateCompetitionRequest("Premier League", "England", "EPL");
    ...
    {
        await using var factory = new CustomWebApplicationFactory(); using var client = factory.CreateClient();
        Assert.Equal(HttpStatusCode.BadRequest, (await client.PostAsJsonAsync("/competitions", new CreateCompetitionRequest("Premier League", "England", ""))).StatusCode);
    }
    ...
    {
        await using var factory = new CustomWebApplicationFactory(); using var client = factory.CreateClient(); Assert.Equal(HttpStatusCode.NotFound, await GetStatus(client, $"/competitions/{Guid.NewGuid()}"));
    }
    ...
    {
        await using var factory = new CustomWebApplicationFactory(); using var client = factory.CreateClient();
        Assert.Equal(HttpStatusCode.BadRequest, (await client.PutAsJsonAsync($"/competitions/{Guid.NewGuid()}", new CreateCompetitionRequest("La Liga", "Spain", "LL"))).StatusCode);
    }
