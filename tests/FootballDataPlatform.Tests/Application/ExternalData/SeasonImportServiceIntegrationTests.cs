...
public sealed class SeasonImportServiceIntegrationTests
{
    ...
    public async Task ImportAsync_ShouldPersistSeasonAndExternalIdentity_AndBeIdempotent()
    {
        ...
        var first = await importService.ImportAsync("PL");
        ...
        Assert.Equal(2, await db.ExternalIdentities.CountAsync());

        var season = await db.Seasons.SingleAsync();
        Assert.Equal(competition.Id, season.CompetitionId);
        Assert.Equal("2025/26", season.Name);
        Assert.Equal(new DateOnly(2025, 8, 15), season.StartDate);
        Assert.Equal(new DateOnly(2026, 5, 24), season.EndDate);

        provider.SetSeasons(new ExternalSeason("2817", "2021", "2025/26", new DateOnly(2025, 8, 15), new DateOnly(2026, 5, 24)));
        var second = await importService.ImportAsync("PL");
    ...
    {
        private IReadOnlyCollection<ExternalSeason> _seasons =
            [new ExternalSeason("2817", "2021", "2025/26", new DateOnly(2025, 8, 15), new DateOnly(2026, 5, 24))];
