...
public sealed class CompetitionImportServiceIntegrationTests
{
    ...
    public async Task ImportAsync_ShouldPersistCompetitionAndExternalIdentity_AndBeIdempotent()
    {
        ...
        var firstResult = await importService.ImportAsync("football-data.org");
        ...
        Assert.Equal(1, await dbContext.Competitions.CountAsync()); Assert.Equal(1, await dbContext.ExternalIdentities.CountAsync());
        var competition = await dbContext.Competitions.SingleAsync();
        Assert.Equal("Premier League", competition.Name); Assert.Equal("England", competition.Country); Assert.Equal("PL", competition.Code);
        var identity = await dbContext.ExternalIdentities.SingleAsync();
        Assert.Equal("football-data.org", identity.Provider); Assert.Equal("Competition", identity.EntityType); Assert.Equal("2021", identity.ExternalId); Assert.Equal(competition.Id, identity.InternalEntityId);

        source.SetCompetitions(new ExternalCompetition("2021", "Premier League Updated", "PL", "England"));
        var secondResult = await importService.ImportAsync("football-data.org");
        ...
        Assert.Equal(competition.Id, updatedCompetition.Id); Assert.Equal("Premier League Updated", updatedCompetition.Name);
    }
    ...
    {
        private IReadOnlyCollection<ExternalCompetition> _competitions = [new ExternalCompetition("2021", "Premier League", "PL", "England")];
        ...
        public Task<IReadOnlyCollection<ExternalCompetition>> GetCompetitionsAsync(CancellationToken cancellationToken = default) => Task.FromResult(_competitions);
