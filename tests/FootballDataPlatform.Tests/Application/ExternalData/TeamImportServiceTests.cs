...
public sealed class TeamImportServiceTests
{
    ...
    public async Task ImportAsync_WhenIdentityDoesNotExist_CreatesTeamAndIdentity()
    {
        ...
        identities.Verify(x => x.AddAsync(It.Is<ExternalIdentityRecord>(i => i.Provider == "football-data.org" && i.EntityType == "Team" && i.ExternalId == "64"), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task ImportAsync_WhenIdentityPointsToMissingTeam_SkipsAndReportsError()
    {
        ...
        var identities = new Mock<IExternalIdentityRepository>();
        var service = new TeamImportService(resolver.Object, teams.Object, identities.Object);
        identities.Setup(x => x.FindAsync("football-data.org", "Team", "64", It.IsAny<CancellationToken>())).ReturnsAsync(new ExternalIdentityRecord("football-data.org", "Team", "64", Guid.NewGuid(), DateTimeOffset.UtcNow));
        teams.Setup(x => x.GetByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>())).ReturnsAsync((Team?)null);
    ...
    {
        var resolver = CreateResolver(new ExternalTeam("", "Liverpool FC", "England"));
        var teams = new Mock<ITeamRepository>();
        var identities = new Mock<IExternalIdentityRepository>();
        var service = new TeamImportService(resolver.Object, teams.Object, identities.Object);
