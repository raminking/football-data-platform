...
{
    private static readonly ExternalCompetition ExternalCompetition = new("2021", "Premier League", "PL", "England");

    [Fact]
    public async Task ImportAsync_WhenIdentityDoesNotExist_CreatesCompetitionAndIdentity()
    {
        var source = CreateSource(); var resolver = CreateResolver(source);
        var competitions = new Mock<ICompetitionRepository>(); var identities = new Mock<IExternalIdentityRepository>();
        identities.Setup(x => x.FindAsync("football-data.org", "Competition", "2021", It.IsAny<CancellationToken>())).ReturnsAsync((ExternalIdentityRecord?)null);
        competitions.Setup(x => x.ExistsByIdentityAsync("Premier League", "England", "PL", null, It.IsAny<CancellationToken>())).ReturnsAsync(false);
        var result = await new CompetitionImportService(resolver.Object, competitions.Object, identities.Object).ImportAsync("football-data.org");
    ...
    {
        var competition = new Competition("Premier League", "England", "PL"); var source = CreateSource(); var resolver = CreateResolver(source);
        var competitions = new Mock<ICompetitionRepository>(); var identities = new Mock<IExternalIdentityRepository>();
        ...
        var result = await new CompetitionImportService(resolver.Object, competitions.Object, identities.Object).ImportAsync("football-data.org");
        Assert.Equal(0, result.Created); Assert.Equal(1, result.Updated); Assert.Equal(0, result.Skipped);
    ...
    {
        var source = CreateSource(); var resolver = CreateResolver(source); var competitions = new Mock<ICompetitionRepository>(); var identities = new Mock<IExternalIdentityRepository>();
        identities.Setup(x => x.FindAsync("football-data.org", "Competition", "2021", It.IsAny<CancellationToken>())).ReturnsAsync(new ExternalIdentityRecord("football-data.org", "Competition", "2021", 999999, DateTimeOffset.UtcNow));
        competitions.Setup(x => x.GetByIdAsync(999999, It.IsAny<CancellationToken>())).ReturnsAsync((Competition?)null);
        var result = await new CompetitionImportService(resolver.Object, competitions.Object, identities.Object).ImportAsync("football-data.org");
    ...
    {
        var source = CreateSource(new ExternalCompetition("", "Premier League", "PL", "England")); var resolver = CreateResolver(source);
        var competitions = new Mock<ICompetitionRepository>(); var identities = new Mock<IExternalIdentityRepository>();
        var result = await new CompetitionImportService(resolver.Object, competitions.Object, identities.Object).ImportAsync("football-data.org");
    ...
    private static Mock<IFootballDataSource> CreateSource(params ExternalCompetition[] competitions)
    {
        ...
        source.Setup(x => x.GetCompetitionsAsync(It.IsAny<CancellationToken>())).ReturnsAsync((IReadOnlyCollection<ExternalCompetition>)(competitions.Length == 0 ? [ExternalCompetition] : competitions)); return source;
    }
