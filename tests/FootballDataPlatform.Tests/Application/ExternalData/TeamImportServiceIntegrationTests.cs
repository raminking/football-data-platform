...
public sealed class TeamImportServiceIntegrationTests
{
    ...
    public async Task ImportAsync_ShouldPersistTeamAndExternalIdentity_AndBeIdempotent()
    {
        ...
        var importService = scope.ServiceProvider
            .GetRequiredService<ITeamImportService>();

        var dbContext = scope.ServiceProvider
            .GetRequiredService<FootballDataDbContext>();

        var provider = scope.ServiceProvider
            .GetRequiredService<IFootballDataProvider>();

        var fakeProvider = Assert.IsType<FakeFootballDataProvider>(provider);
        ...
        fakeProvider.SetTeams(
            new ExternalTeam("64", "Liverpool", "England"));
        ...
        Assert.Equal(team.Id, unchangedIdentity.InternalEntityId);
    }
    ...
    {
        private IReadOnlyCollection<ExternalTeam> _teams =
            [new ExternalTeam("64", "Liverpool FC", "England")];

        public string ProviderName => "football-data.org";

        public void SetTeams(params ExternalTeam[] teams)
        {
            _teams = teams;
        }
